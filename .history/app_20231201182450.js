...
app.get("/add/List", async function (req, res) {
    const nameOfList = req.params.nameOfList;

    const result = await List.findOne({ name: nameOfList }).exec();
    if (result === null) {
        const list = new List({
            name: nameOfList,
            item: defaultItems
        ...
        res.render("toDoList", { listTitle: nameOfList, newListItems: result.item });
    };
});

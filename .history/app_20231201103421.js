...
//     app.locals.customListName = req.params.customListName;
//     const customListName = app.locals.customListName;
//     next();
...
app.post("/:customListName", async function (req, res) {
    ...
    if (!list) {
        const newList = new List({
            name: customListName,
            item: [{ name: itemName }] // Convert itemName to an object
        });

        await newList.save();
        res.redirect("/" + customListName);
    } else {
        list.item.push({ name: itemName }); // Push a new object into the item array
        await list.save();
        res.redirect("/" + customListName);
    }
